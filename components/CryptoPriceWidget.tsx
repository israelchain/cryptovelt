...
export default function CryptoPriceWidget() {
  ...
  return (
    <section
      ...
      className="card rounded-3xl p-6 max-w-2xl mx-auto"
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <h3 className="text-xl font-bold text-ink-900 flex-1">מחירי קריפטו בזמן אמת</h3>
        {lastUpdated && (
        ...
        <button
          ...
          className="border border-slate-200 rounded-lg px-3 py-1 text-base hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {fetching ? '⏳' : '🔄'}
        </button>
      </div>
      ...
      {/* Initial loading */}
      {fetching && !prices && (
        <p className="text-center text-ink-400 py-8">טוען מחירים...</p>
      )}
      ...
      {prices && (
        ...
        >
          <div
            role="row"
            className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr] bg-slate-50 px-4 py-2 text-xs font-semibold text-ink-500 border-b border-slate-200 text-right"
          >
            ...
            return (
              ...
              >
                <span role="cell" className="flex flex-col gap-0.5">
                  <strong className="text-sm text-ink-900">{coin.hebrewName}</strong>
                  <span className="text-xs text-ink-400 font-medium">{coin.symbol}</span>
                </span>
                <span role="cell" className="text-sm font-semibold text-ink-700 tabular-nums">
                  {fmtUSD(d.usd)}
                </span>
                <span role="cell" className="text-sm font-semibold text-ink-700 tabular-nums">
                  {fmtILS(d.ils)}
      ...
      <details className="border border-slate-200 rounded-2xl mb-4 overflow-hidden">
        <summary className="px-4 py-3 text-sm font-semibold text-brand-700 cursor-pointer bg-slate-50 select-none hover:bg-slate-100 transition-colors">
          מה המחירים האלה אומרים? לחץ להסבר
        </summary>
        <div className="px-5 py-4 text-sm leading-relaxed text-ink-500 space-y-3">
          <p>
            <strong className="text-ink-800">מה זה מחיר קריפטו?</strong>{' '}
          ...
          <p>
            <strong className="text-ink-800">למה המחיר משתנה כל הזמן?</strong>{' '}
          ...
          <p>
            <strong className="text-ink-800">ILS — שקל ישראלי:</strong>{' '}
            המחיר בשקלים מחושב לפי שער ההמרה הנוכחי של CoinGecko ומעודכן בזמן אמת.
          </p>
          <p className="bg-btc-50 border-s-4 border-btc-500 px-3 py-2 rounded-e text-btc-800 text-xs">
